...
def extract_video(input_dir, model, scale=1.3, gp=10):
...
def parse_args():
    ...
    parser.add_argument('--video_root_path', type=str,
                        default='/home/biomedia4n6/Public/linyz/datasets/Celeb-DF-v2/video')
    parser.add_argument('--image_root_path', type=str, default='/home/biomedia4n6/Public/linyz/datasets/CDF-face')
...
def main():
    ...
    real_video_path = os.path.join(video_root_path, 'Celeb-real')
    image_path = os.path.join(image_root_path, 'Celeb-real')
    ...
    for idx in tqdm(range(len(real_video_name))):
        ...
        real_video_path = os.path.join(video_root_path, 'Celeb-real', real_video_name[idx])
        face_images, face_boxes, face_index = extract_video(real_video_path, model, scale=scale)

        output_path = os.path.join(image_root_path, 'Celeb-real', real_video_name[idx])
        ...
        for a in range(0, id_num + 1):
            fake_video_name = real_video_name[idx].split('_')[0] + '_id%d_' % a + real_video_name[idx].split('_')[1]
            fake_video_path = os.path.join(video_root_path, 'Celeb-synthesis', fake_video_name)

            if os.path.isfile(fake_video_path):

                output_path = os.path.join(image_root_path, 'Celeb-synthesis', fake_video_name)
                if not os.path.isdir(output_path):
                    os.mkdir(output_path)

                reader = cv2.VideoCapture(fake_video_path)
                frames_num = int(reader.get(cv2.CAP_PROP_FRAME_COUNT))
                for j in range(frames_num):
                    ...
                    if j in face_index:
                        ...
                        cv2.imwrite(os.path.join(output_path, "%04d.png" % j), face)
