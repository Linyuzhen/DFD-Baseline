...
def extract_video(input_dir, model, scale=1.3, smallest_h=0, smallest_w=0):
...
def get_smallest_hw(video_root_path, real_sub_path, real_video, deepfake_sub_path, fake_video):
    ...
    reader = cv2.VideoCapture(os.path.join(video_root_path, deepfake_sub_path.replace('Deepfakes', 'FaceShifter'), fake_video))
    reader.grab()
    success, frame = reader.retrieve()
...
def main(video_root_path, image_root_path, real_sub_path, deepfake_sub_path, real_videos, fake_videos):
    ...
    for idx in tqdm(range(len(real_videos))):
        ...
        reader = cv2.VideoCapture(os.path.join(video_root_path, deepfake_sub_path.replace('Deepfakes', 'Face2Face'), fake_videos[idx]))
        ...
        reader = cv2.VideoCapture(os.path.join(video_root_path, deepfake_sub_path.replace('Deepfakes', 'NeuralTextures'), fake_videos[idx]))
        frames_num = int(reader.get(cv2.CAP_PROP_FRAME_COUNT))
        ...
        reader = cv2.VideoCapture(os.path.join(video_root_path, deepfake_sub_path.replace('Deepfakes', 'FaceSwap'), fake_videos[idx]))
        frames_num = int(reader.get(cv2.CAP_PROP_FRAME_COUNT))
        ...
        reader = cv2.VideoCapture(os.path.join(video_root_path, deepfake_sub_path.replace('Deepfakes', 'FaceShifter'), fake_videos[idx]))
        frames_num = int(reader.get(cv2.CAP_PROP_FRAME_COUNT))
